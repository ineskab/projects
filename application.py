...
def change_passwords():
    ...
    if request.method == "POST":
        ...
        current_hash = db.execute("SELECT hash FROM users WHERE id =:id", id = session["user_id"])
        ...
        db.execute("UPDATE users SET hash =:hash WHERE id=:id", id = session["user_id"], hash = generate_password_hash(request.form.get("new_password")))
